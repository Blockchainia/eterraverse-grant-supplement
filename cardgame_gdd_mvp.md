...
1. **Ranks:** Each card is assigned four ranks, one in each Cardinal direction, when the pack is opened. These cannot be altered or levelled. These Ranks only effect the cards usefulness in the Card game, and will not effect its value or ability in future games. 

1. **Attack Class:** Each card is assigned an Attack Class when a pack is opened. This will be either Magic or Physical. The class can be altered by using a special item. 

1. **Stats:** Each card contains an HP, Attack, and Defense statistic. When a pack is opened the player will be able to pull a slot mechanism that will randomly assign values to each of these statistics. The range for each statistic is 1 through 15. 

### Mint Process

1. **Purchase:** The player will be able to purchase a pack for the equivalent of $2.99 USD. The player will be able to purchase 6 packs for 12.99 USD. 

1. **Card Forge:** After purchase the player will proceed to The Forge where the card values can assigned and the final cards minted.

1. **Max Forge:** A Max Forge is a scenario when a Player Forges a card with >=5 for each stat, allowing them to fully level up a card to 15 in each Stat through assigning experience to that card. When all stats have been leveled up to 15, the Card is said to be Maxed. 

1. **Rarity:** Each card will be assigned a Rarity from it's unique genetics that will determine its effectiveness in future games. 

1. **Dice Roll:** Assigns the number of slot pulls for a card. For Common, Rare, and Epic cards, 1 dice is rolled. For Legendary cards, two dice are rolled. 

1. **Slots:** For each card, the player will roll dice according to the Rarity above. The dice roll will determine the number of Slot pulls allowed before assigning the card's initial Stats. Once the slot is pulled, the player can no longer use the values from a previous pull. The player does not need to use all of the slot pulls. 

1. **Slot Values:** The slot machine will contain three spinners, each with the values 1-7, and an Eterraverse logo representing the number 10. 

1. **Slot Probability:** Below are the probability of a successful Max Forge. 
    * Legendary Cards Roll two dice
    * Success is defined as having a card with >=5 in each Stat. This will allow the Player to level up the card fully to 15 for all Stats using experience.

        | Number of Pulls | Probability of Success |
        |-----------------|------------------------|
        | 1               | 0.125000               |
        | 2               | 0.234375               |
        | 3               | 0.330078               |
...
1. **Card Levels:** Each card begins at Level 0 and can be levelled up to Level 30. 

1. **Experience:** Every time a Card is used in a match the Card earns experience. When a match is won, the Player also earns experience. Experience earned by the player can be assigned to any card they choose to increase its Level. 

1. **Increasing Stats:** Every time a Card's Level increases, the player can increase one stat on the card. 

1. **Maxed Stats:** Players can only level up a card 30 times. Stats can only go up to 15. When a card reached a 15 stat for all three stats, it is said to be "Maxed."

### Game Setup
1. **Ranks:** Each card has four numbers, one in each Cardinal Direction, representing its strength in that direction. Each number is referred to as a Rank, evaluated as a Value from one to ten, ten being represented by the letter 'A'. A cards Ranks are permanent, set when the card is minted. 

1. **Stats:** Each card contains unique Genetics that derive it's initial Face Values, Brawl Arrows, Attributes, and Elemental compatibility. These genetics play a crucial role in the Brawl Mathematics through each Genetic attribute's value, such as setting the Card's Brawl Class and Power attribute. 

1. **Card Ranks:** Cards have four Ranks which comprise it's Face Values, one in each Cardinal direction. These Ranks only apply to the Card Game
